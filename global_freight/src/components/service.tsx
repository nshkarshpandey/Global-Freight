...
export default function Service() {
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex flex-wrap justify-center gap-52 serv-card">
        {images.map((img, index) => (
          ...
          >
            <div className="relative w-[25rem] h-[25rem] serv-img ">
              <Image
            ...
            <div className="text-left mt-2 text-black w-[136%] serv-txt">
               <h3 className={`serv-head text-xl font-semibold transition-colors duration-300 ${hoveredIndex === index ? 'text-[#ec7620]' : 'text-black'}`}>{img.title}</h3>
              <p className="serv-des text-sm">{img.description}</p>
            </div>
          </div>
        ))}
      </div>
